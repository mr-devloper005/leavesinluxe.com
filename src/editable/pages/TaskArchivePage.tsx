...
const cardBase = 'group block border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_8px_28px_rgba(0,0,0,0.08)]'
      ...
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="border-b border-[var(--tk-line)] bg-[var(--tk-surface)]">
          <div className="mx-auto max-w-[var(--editable-container)] px-6 py-16 sm:py-20 lg:px-8">
            <p className="text-sm font-medium text-[var(--tk-accent)]">{theme.kicker}</p>
            <h1 className="editable-display mt-3 max-w-3xl text-4xl leading-[1.08] sm:text-5xl lg:text-6xl">
            ...
            {voice?.chips?.length ? (
              <div className="mt-6 flex flex-wrap gap-2">
                {voice.chips.map((chip) => (
                  <span key={chip} className="border border-[var(--tk-line)] bg-[var(--tk-bg)] px-3 py-1.5 text-xs font-medium text-[var(--tk-muted)]">{chip}</span>
                ))}
              </div>
            ) : null}
            ...
            <div className="mt-10 flex flex-col gap-4 border-t border-[var(--tk-line)] pt-6 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-[var(--tk-muted)]">
                <span className="font-medium text-[var(--tk-text)]">{posts.length}</span> {posts.length === 1 ? 'post' : 'posts'} · {categoryLabel}
              </p>
              <form action={basePath} className="flex items-center gap-2.5">
                <div className="relative">
                  <select
                    ...
                    className="h-10 appearance-none border border-[var(--tk-line)] bg-[var(--tk-surface)] pl-4 pr-10 text-sm font-medium text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
                ...
                <button className="inline-flex h-10 items-center bg-[var(--tk-accent)] px-5 text-sm font-medium text-white transition hover:opacity-90">Apply</button>
              </form>
            </div>
          </div>
        </header>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-6 py-14 sm:py-16 lg:px-8">
          {posts.length ? (
            ...
            <div className="mx-auto max-w-xl border border-dashed border-[var(--tk-line)] bg-[var(--tk-surface)] px-8 py-16 text-center">
              ...
              <h2 className="editable-display mt-4 text-2xl">Nothing here yet</h2>
          ...
          {posts.length ? (
            <nav className="mt-14 flex items-center justify-center gap-3 text-sm">
              ...
              <span className="border border-[var(--tk-line)] bg-[var(--tk-surface)] px-5 py-2.5 font-medium text-[var(--tk-muted)]">Page {page} of {pagination.totalPages || 1}</span>
...
function CardArrow({ label }: { label: string }) {
  return (
    <span className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-[var(--tk-accent)]">
      {label}
...
function ArticleArchiveCard({ post, href, index: _index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className={`${cardBase} overflow-hidden`}>
      ...
      <div className="p-5 sm:p-6">
        <span className="text-xs font-medium text-[var(--tk-accent)]">{category}</span>
        <h2 className="editable-display mt-2 text-xl leading-snug">{post.title}</h2>
  ...
  return (
    <Link href={href} className={`${cardBase} flex items-center gap-5 p-5 sm:p-6`}>
      ...
      <div className="min-w-0 flex-1">
        <h2 className="editable-display truncate text-lg">{post.title}</h2>
        <p className="mt-1.5 line-clamp-1 text-sm leading-6 text-[var(--tk-muted)]">{getSummary(post)}</p>
        <div className="mt-2.5 flex flex-wrap gap-3 text-xs font-medium text-[var(--tk-muted)]">
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col p-5 sm:p-6`}>
      <div className="flex items-start justify-between gap-4">
        <span className="editable-display text-2xl text-[var(--tk-accent)]">{price || 'Open offer'}</span>
        {condition ? <span className="bg-[var(--tk-accent-soft)] px-3 py-1 text-[11px] font-medium uppercase tracking-[0.1em] text-[var(--tk-accent)]">{condition}</span> : null}
      </div>
      <h2 className="editable-display mt-4 text-lg leading-snug">{post.title}</h2>
      <p className="mt-2 line-clamp-3 flex-1 text-sm leading-6 text-[var(--tk-muted)]">{getSummary(post)}</p>
...
function ImageArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  const image = getImage(post)
  return (
    <Link href={href} className="group mb-5 block break-inside-avoid overflow-hidden border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-300 hover:-translate-y-0.5">
      <div className={`relative overflow-hidden ${index % 3 === 0 ? 'aspect-[3/4]' : 'aspect-[4/3]'}`}>
        ...
        <div className="absolute inset-x-0 bottom-0 p-4">
          <h2 className="editable-display line-clamp-2 text-base leading-snug text-white">{post.title}</h2>
          <span className="mt-1.5 inline-flex items-center gap-1 text-xs font-medium text-white/60">View <ArrowUpRight className="h-3 w-3" /></span>
        </div>
      </div>
    </Link>
  )
...
function BookmarkArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex gap-4 p-5`}>
      <div className="flex h-11 w-11 shrink-0 items-center justify-center bg-[var(--tk-accent-soft)] text-[var(--tk-accent)]">
        <Globe className="h-5 w-5" />
      </div>
      <div className="min-w-0 flex-1">
        <span className="text-[11px] font-medium uppercase tracking-[0.14em] text-[var(--tk-muted)]">Saved · {String(index + 1).padStart(2, '0')}</span>
        <h2 className="editable-display mt-1 text-base leading-snug">{post.title}</h2>
        <p className="mt-1.5 line-clamp-2 text-sm leading-6 text-[var(--tk-muted)]">{getSummary(post)}</p>
        {website ? <p className="mt-2 truncate text-xs font-medium text-[var(--tk-accent)]">{cleanDomain(website)}</p> : null}
      </div>
    </Link>
  )
}
...
function PdfArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col p-5 sm:p-6`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex h-11 w-11 items-center justify-center bg-[var(--tk-accent-soft)] text-[var(--tk-accent)]"><FileText className="h-5 w-5" /></div>
        <span className="border border-[var(--tk-line)] px-3 py-1 text-[11px] font-medium uppercase tracking-[0.1em] text-[var(--tk-muted)]">{category}</span>
      </div>
      <h2 className="editable-display mt-5 text-lg leading-snug">{post.title}</h2>
      <p className="mt-2 line-clamp-3 flex-1 text-sm leading-6 text-[var(--tk-muted)]">{getSummary(post)}</p>
      <span className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-[var(--tk-accent)]">Open document <Download className="h-4 w-4" /></span>
    </Link>
  )
}
...
function ProfileArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col items-center p-6 text-center`}>
      ...
      <h2 className="editable-display mt-4 text-base">{post.title}</h2>
